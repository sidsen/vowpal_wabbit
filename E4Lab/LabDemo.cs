...
{
    public INTERACTION[] full_interactions;
    ...
    public void Run()
    {
        MwtExplorer mwt = new MwtExplorer("myservice",null);
        ...
        IOUtils iou = new IOUtils("speller-contexts", "speller-rewards");
        ...
        interactions = mwt.GetAllInteractions();

        MwtRewardReporter rewardReporter = new MwtRewardReporter(interactions);
        float rewardSum = 0.0f;
        RewardStore rewardStore = new RewardStore("old-rewards");
        MwtLogger interactionLogger = new MwtLogger("old-contexts");
        interactionLogger.Add(interactions);
        ...
        rewardStore.Flush();
        Console.WriteLine("Average reward = {0}", rewardSum / interactions.Length);

        full_interactions = rewardReporter.GetAllInteractions();
        ...
        for (float param = 0.05f; param <= 1; param += 0.05f)
            Console.WriteLine("Value of default policy and threshold {0} = {1}", param, mwtopt.EvaluatePolicy<float>(new StatefulPolicyDelegate<float>(ScoreBasedPolicy), param));
