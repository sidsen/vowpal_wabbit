...
static std::map<BucketId, std::string> BucketIdMapA = {{BucketX7, "X7"}, {BucketX6, "X6"}, {BucketX5, "X5"},
    {BucketX4, "X4"}, {BucketX3, "X3"}, {BucketX2, "X2"}, {BucketX1, "X1"}, {Bucket0, "0"}, {Bucket1, "1"},
    {Bucket2, "2"}, {Bucket3, "3"}, {Bucket4, "4"}, {Bucket5, "5"}, {Bucket6, "6"}};

static std::map<BucketId, std::wstring> BucketIdMap = {{BucketX7, L"X7"}, {BucketX6, L"X6"}, {BucketX5, L"X5"},
    {BucketX4, L"X4"}, {BucketX3, L"X3"}, {BucketX2, L"X2"}, {BucketX1, L"X1"}, {Bucket0, L"0"}, {Bucket1, L"1"},
    {Bucket2, L"2"}, {Bucket3, L"3"}, {Bucket4, L"4"}, {Bucket5, L"5"}, {Bucket6, L"6"}};

struct CpuWaitTimeBucketCounters
{
  ~CpuWaitTimeBucketCounters() { PdhCloseQuery(query); }
  ...
  HRESULT ComputeNumVcpus(const std::wstring &VmName)
  {
    ...
    CHECK_EQ(ERROR_SUCCESS,
        PdhGetFormattedCounterValue(counter, PDH_FMT_LARGE | PDH_FMT_NOCAP100, &CounterType, &DisplayValue));
  ...
  HRESULT init(const std::wstring &VmName)
  {
    ...
    for (size_t vp = 0; vp < NumVcpus; vp++)
    {
      ...
      for (auto bucket : BucketIdMap)
      {
        ...
        std::wstring path =
            prefix + VmName + L":hv vp " + std::to_wstring(vp) + L")\\CPU Wait Time Bucket " + bucket.second;
    ...
    for (auto bucket : BucketIdMap)
    {
      ...
      for (size_t vp = 0; vp < NumVcpus; vp++)
      {
        PDH_STATUS status = PdhGetFormattedCounterValue(
            CounterHandles[vp][bucketId], PDH_FMT_DOUBLE | PDH_FMT_NOCAP100, &CounterType, &DisplayValue);
        if (status != ERROR_SUCCESS)
        {
          std::wcout << "Error quering counter '" << CounterPaths[vp][bucketId] << "': 0x" << std::hex << status
                     << std::endl;
          return E_FAIL;
        }
...
{
  HRESULT init(
      Mode _mode, std::wstring _vmName, INT32 _numCores, BOOLEAN _primary = FALSE, BOOLEAN _disjointCpuGroups = FALSE)
  {
    mode = _mode;
    curCores = _numCores;
  ...
  HRESULT addCores(UINT32 numCores, CPU_SET mask)
  {
    ...
    if (!primary)
    {
      ...
      {
        wprintf(
            L"Error adding cores to secondary; mask 0x%lx, new curCoreMask: 0x%lx: 0x%lx\n", mask, curCoreMask, Result);
        return Result;
      }
    }
    return S_OK;
  ...
  UINT64 busyCoreMask(CPU_SET systemBusyMask) { return systemBusyMask & curCoreMask; }

  UINT32 busyCores(CPU_SET systemBusyMask) { return HVMAgent_CoreCount(busyCoreMask(systemBusyMask)); }
